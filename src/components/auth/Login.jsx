...
function Login(props) {
  ...
  return (
    <div
      className={`container bg-light-gray flex justify-center align-center ${getDarkClass(
        "bg-dark"
      )}`}
    >
      <div className={`login_form ${getDarkClass("dark-accent")}`}>
        <div
          className={`user_img flex justify-center align-center ${getDarkClass(
            "dark-indigo"
          )}`}
          style={{ backgroundColor: "white" }}
        >
          {/* <PersonRoundedIcon fontSize="large" /> */}
          <img style={{ widows: 90, height: 90 }} src={logo} alt="logo" />
        </div>
        <div className="form_content">
          <h3
            className={`text-center mt-60 mb-20 ${getDarkClass("dark-white")}`}
          >
            Admin Log in
          </h3>
          <Form
